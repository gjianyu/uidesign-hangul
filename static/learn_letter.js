...
function show_info(info){
    ...
    $("#form").submit(function(event){
        ...
        if(stats["end"]=="1" && isCorrect=="Correct!"){
            $("#change-state").append("<a class = 'p-3 prev next-button btn btn-light' href='"+ prev_id +"'>← PREVIOUS</a>")
            $("#change-state").append("<a class = 'p-3 prev next-button btn btn-light' href = '../../quiz/class1/letter'>NEXT →</a></div>")
        }
        else if(prev_id == 0 && isCorrect=="Correct!"){
            $("#change-state").append("<a class = 'p-3 prev next-button btn btn-light' href='#'>← PREVIOUS</a>")
            $("#change-state").append("<a class = 'p-3 prev next-button btn btn-light' href = '" + next_id+"'>NEXT →</a></div>")
        }
